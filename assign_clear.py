...
MONTHLY_DAYS = 20 #기본 통할 일수
F_TRANSFER_PENALTY = 0.15 #환승 1회당 패널티
F_HEADWAY_PENALTY = 0.2 #배차간격 패널티
HEADWAY_THRESHOLD = 15 #배차간격 패널티 시간
WCBI_WEIGHT = 0.7  #통학거리 가중치
GPA_WEIGHT = 0.3   #성적 가중치
'''
'''
비용 패널티 삭제 예정
...
def calculate_score_to30(score):
    '''
...
def calculate_wcbi_score(route):
    ...
    api_payment = info.get('totalPayment')
...
def main():
    ...
    try:
        ...
        df_final['통학 점수(70점)'] = df_final['WCBI_최종점수'].rank(method='dense', pct=True) * WCBI_WEIGHT*100 #환산식 수정 260115
        # 신입생 1000점 / 재학생 4.5점 자동 구분 함수 적용
        df_final['성적 점수(30점)'] = df_final[gpa_col].apply(calculate_score_to30) 
        df_final['Final_Score (최종 합산점)'] = (df_final['통학 점수(70점)']) + (df_final['성적 점수(30점)'] )
        ...
        out_cols = list(df_students.columns) + [
            '배정결과', '배정방식', '배정된 방', '금액', 
            'Final_Score (최종 합산점)', '통학 점수(70점)', '성적 점수(30점)',
        ...
        output_name = f'기숙사_배정_결과_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx'
